...
if __name__ == "__main__":
    ...
    columns = list(name_columns.keys())
        ...
        with zip.ZipFile(folder + file, 'r') as zip_file:
            for name in zip_file.namelist():
                if name.find('MICRODADOS_CADASTRO_CURSOS_') > 0:
                    ...
                    ft_oferta_curso = df_tmp[columns]
                    ft_oferta_curso = ft_oferta_curso.rename(columns=name_columns)
                    ft_oferta_curso.fillna(0, inplace=True)
                    year = name[-8:-4]
                    new_data.execute_query(f'DELETE FROM `EDUCADADOS`.`FT_OFERTA_CURSO` WHERE NR_ANO = {year};')
